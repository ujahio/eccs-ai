...
- Demonstration-only deployments may enable teacher-only lifecycle override controls with `CASE_LIFECYCLE_DEMO_CONTROLS_ENABLED`. When enabled, teachers may archive the active dashboard case with past lifecycle dates or restore an archived/expired case from All Cases as the single active published case with a 14-day UAE-time deadline window. `/teacher/cases` must still exclude the active published case and list only drafts and archived cases. This override is for demos only and does not change the long-term v1 product rule that published case lifecycle is otherwise immutable.
