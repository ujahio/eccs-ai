...
`package.json` pins Bun 1.3.10, Next.js 16.2.12, React 19.2.8, TypeScript 6.0.3, SST 4.17.1, Better Auth 1.6.25, AWS SDK clients 3.1095.0, Resend 6.18.0, Tailwind 4.3.3, Vitest 4.1.10, Vite 8.1.5 and Playwright 1.62.0. These are repository declarations, not claims about newer upstream versions. `bun.lock` records dependency resolution; package overrides pin selected transitive packages. `README.md`, `.node-version`, and `.nvmrc` identify Node 24 for the toolchain. Bun is the project command/package runtime; individual Lambda runtime versions are not explicitly set in the inspected component definitions.
